...
ADDITIONAL_UNIT_CONVERSION = [
    ...
    ('litre', 'cubic meter', 1e-3)  # I don't know why but this one is commented in BW source code
]

unit_conversion = {(conversion[0], conversion[1]): conversion[2]
...
VALID_UNITS = set([x for x in UNITS_NORMALIZATION.keys()]) \
...
def normalize_units(unit: str) -> str:
    ...
    """
    return UNITS_NORMALIZATION.get(unit.lower(), unit.lower() if unit.lower() in UNITS_NORMALIZATION.values() else unit)
        ...
        for exchange in activity['exchanges']:
            ...
            if exchange['input'] in activities.keys():
                if normalize_units(exchange['unit']) not in VALID_UNITS:
                    raise ValueError(f"Invalid unit \"{exchange['unit']}\" for exchange \"{exchange['input']}\"")
            ...
            if input_unit != reference_unit:
                if normalize_units(input_unit) != normalize_units(reference_unit):
                    try:
                        exchange['amount'] = exchange['amount'] * unit_conversion[normalize_units(input_unit),
                                                                                  normalize_units(reference_unit)]
                    except KeyError:
                        raise KeyError(f'Unit conversion missing: '
